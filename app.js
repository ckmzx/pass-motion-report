...
// 2. 비디오 파일 로드 처리 (코덱 디코딩 실패 시 안전 폴백)
function handleVideoFile(file) {
  ...
  videoPlayer.onerror = function() {
    showAlert('⚠️ 브라우저가 해당 영상 코덱(MPEG-4 Part 2)을 직접 재생하지 못했습니다. (CSV 데이터로 분석을 계속 진행합니다)');
    videoLoaded = false;
  ...
  Papa.parse(file, {
    ...
    complete: function(results) {
      if (results.data && results.data.length > 0) {
        ...
        showAlert(`✓ CSV 실측 데이터 (${parsedCsvRows.length}개 프레임)가 로드되었습니다!`);
        triggerFullEvaluation();
      } else {
        alert('CSV 파일에서 데이터를 읽지 못했습니다.');
      }
    }
  });
}
...
async function triggerFullEvaluation() {
  ...
  const duration = (videoPlayer && !isNaN(videoPlayer.duration) && videoPlayer.duration > 0) ? videoPlayer.duration : 5.0;
  let kinematics = {};
  ...
  if (parsedCsvRows) {
    ...
    kinematics = {
      p1: { time: Math.min(0.1, duration * 0.05), knee: (parsedCsvRows[0].left_knee || 172.5).toFixed(1), hip: (parsedCsvRows[0].left_hip || 168.0).toFixed(1) },
  ...
  if (videoLoaded) {
    ...
    // 비디오 코덱 미지원 시 안전 그래픽 배치
    for (let p = 1; p <= 5; p++) {
      const imgElem = document.getElementById(`phaseImg${p}`);
      if (imgElem) imgElem.src = generateFallbackGraphic(kinematics[`p${p}`].time, p);
    }
  }
  ...
  return new Promise((resolve) => {
    ...
    const doCap = () => {
      ...
      try {
        ...
        resolve(generateFallbackGraphic(timeSec, 1));
      }
    };
...
function generateFallbackGraphic(timeSec, phase) {
  const titles = ["", "준비 자세", "최대 굴곡 (최저점)", "이륙 순간 (Take-off)", "공중 체공 (Tuck)", "착지 및 완충"];
  ...
  ctx.fillText(`PHASE 0${phase} · ${titles[phase] || ''}`, 240, 125);
  ctx.fillStyle = '#8b949e';
  ctx.font = '13px Pretendard, sans-serif';
  ctx.fillText(`시간: ${timeSec.toFixed(2)}s | 2D Motion Kinematics`, 240, 155);
...
function renderDynamicReport(k) {
  ...
  if (minKneeNum >= 70 && minKneeNum <= 85) {
    ...
    fbText2.innerHTML = `실측 최소 무릎각이 <strong>${minKneeNum}°</strong>로 대퇴사두근과 둔근의 <strong>신장-단축 주기(SSC)</strong> 탄성에너지를 극대화하기에 가장 이상적인 깊이를 형성했습니다.`;
    ...
    fbText2.innerHTML = `실측 무릎각이 <strong>${minKneeNum}°</strong>로 충분히 앉지 않아 하체 탄성에너지를 100% 활용하지 못했습니다.`;
  }
  ...
  if (extKneeNum >= 160) {
    ...
    fbText3.innerHTML = `이륙 시 발목-무릎-고관절이 <strong>${extKneeNum}°</strong>까지 완벽히 신전되는 <strong>트리플 익스텐션(Triple Extension)</strong>을 달성했습니다.`;
  } else {
    statusKnee3.className = 'm-status status-warn';
    statusKnee3.innerText = '조기 무릎 굽힘 (추진력 손실)';
    fbText3.innerHTML = `이륙 순간 무릎 신전각이 <strong>${extKneeNum}°</strong>로 지면을 끝까지 밀지 못하고 조기에 무릎을 접었습니다.`;
  }
