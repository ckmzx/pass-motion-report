# ==============================================================================
# 2D 마커리스 제자리멀리뛰기 영상 분석 파이프라인 (H.264 웹 표준 코덱 강제 적용)
# ==============================================================================
...
# 2. 파이썬 기반 비디오 분석 코어 엔진 (코덱 호환성 대폭 강화)
py_run_string("
import cv2
import numpy as np
import pandas as pd
...
def process_jump_video_core(video_path, out_video_path, out_csv_path):
    ...
    # 웹 브라우저 100% 호환 H.264 / MP4V 코덱 설정
    fourcc_options = ['avc1', 'H264', 'mp4v']
    out = None
    
    for c in fourcc_options:
        try:
            fourcc = cv2.VideoWriter_fourcc(*c)
            temp_out = cv2.VideoWriter(out_video_path, fourcc, fps, (width, height))
